...
#             # TODO: Replace with DataJointError
#             raise Exception(
#                 "Connection plugin '{}' not found.".format(plugin_name))
#     else:
#         return host_input
...
class Connection:
    ...
    def __init__(self, host, user, password, port=None, init_fun=None, use_tls=None):
        ...
        self.conn_info = dict(host=host, port=port,
        ...
        if self.is_connected:
            ...
            self.connection_id = self.query(
                'SELECT connection_id()').fetchone()[0]
        else:
    ...
    def purge_query_cache(self):
        ...
        if 'query_cache' in config and isinstance(config['query_cache'], str) and pathlib.Path(config['query_cache']).is_dir():
