...
class Config(collections.MutableMapping):
    ...
    class __Config:
        ...
        def __setitem__(self, key, value):
            logger.log(logging.INFO, u"Setting {0:s} to {1:s}".format(
                str(key), str(value)))
            if validators[key](value):
                self._conf[key] = value
            else:
                # raise DataJointError(
                #     u'Validator for {0:s} did not pass'.format(key))
...
config = Config()
# config_files = (os.path.expanduser(n)
#                 for n in (LOCALCONFIG, os.path.join('~', GLOBALCONFIG)))
# try:
